...
public class CalendarController {
    ...
    @Operation(summary = "Haftalık ders programı",
            description = "Tarihe açılmamış haftalık ders saatleri: gün, başlangıç/bitiş saati, derslik, ders, grup ve hoca. "
                    ...
                    + "staffId ile tek bir hocanın programına, program ile tek bir öğretim programına (UNDERGRADUATE, MASTERS_THESIS, MASTERS_NON_THESIS, DOCTORATE) daraltılır. "
                    + "Her slot dersin programs listesini taşır.")
    @GetMapping("/weekly")
    public ResponseEntity<DataResult<WeeklyScheduleDto>> getWeeklySchedule(
        ...
        var schedule = calendarService.getWeeklySchedule(academicYear, semester, term, staffId, program);
