package com.matmuh.matmuhsite.business.constants;

import com.matmuh.matmuhsite.entities.Announcement;

import java.util.List;

public class AnnouncementMessages {
    public static String announcementAddSuccess = "Duuyuru başarıyla eklendi!";
    public static String getAnnouncementsSuccess = "Duyurular başarıyla getirildi!";
    public static String getAnnouncementByIdSuccess = "Duyuru başarıyla getirildi!";
    public static String nameCanotBeNull = "Duyuru adı boş olamaz!";
    public static String contentCanotBeNull = "Duyuru içeriği boş olamaz!";
    public static String annoucementsNotFound = "Duyurular bulunamadı!";
}
